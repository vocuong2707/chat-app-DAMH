...
const newGroups = async (req, res) => {
  ...
  try {
    ...
    if (members.length < 1) {
      // Nếu số lượng thành viên ít hơn 2
      return res
        .status(400)
        .json({ message: "Nhóm cần có ít nhất 2 thành viên." });
    }
    ...
    if (newAvatar === undefined || newAvatar === null || newAvatar === "") {
      newAvatar =
        "https://mys3hoaian.s3.ap-southeast-1.amazonaws.com/group.png";
    }
...
const getGroupList = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).populate({
      path: "groups",
      select: "name avatar members leader coLeader",
    });

    const newGroups = await Promise.all(
      user.groups.map(async (group) => {
        if (group.members.length === 2) {
          const otherUserId = group.members.filter((mem) => {
            return !mem.equals(userId);
          })[0];
    ...
    for (const memberId of memberIds) {
      if (!group.members.includes(memberId)) {
        ...
        await Messages.create({
          message: { text: `${user.fullname} đã được thêm vào nhóm` },
...
const getNonGroupFriends = async (req, res, next) => {
  try {
  ...
  try {
    ...
    for (const memberId of memberIds) {
      ...
      if (index !== -1) {
        ...
        const user = await User.findById(memberId);
        await Messages.create({
          message: { text: `"${user.fullname}" đã bị xóa khỏi nhóm` },
          users: [memberId, groupId],
          group: groupId,
          sender: group.leader,
          avatar: group.avatar,
...
const setCoLeader = async (req, res, next) => {
  try {
    ...
    if (group.coLeader.length >= 2) {
      return res
        .status(400)
        .json({ success: false, message: "Nhóm đã có đủ 2 nhóm phó" });
    }

    // Kiểm tra xem người dùng đã là nhóm phó chưa
    if (group.coLeader.includes(memberId)) {
      return res.status(400).json({
        success: false,
    ...
    group.coLeader.push(memberId);
    await Messages.create({
      message: { text: `"${user.fullname}" đã trở thành nhóm phó` },
      users: [memberId, groupId],
      group: groupId,
      sender: group.leader,
      avatar: group.avatar,
...
const transferOwnership = async (req, res, next) => {
  try {
    ...
    await Messages.create({
      message: { text: `"${newOwner.fullname}" đã trở thành nhóm trưởng` },
